...
tree_file  <- "iqtree2/afu_afis_aoerl/aft2_transposases.treefile"
...
taxonomy  <- "/home/ailtonpcf/draco/proj/02-compost-microbes/doc/14-phylogeny-accessions/fumigatus_cli_env_test.tsv"
...
left_join(tax_meta)  %>% 
mutate(species = if_else(is.na(species), label, species))
...
library(randomcoloR)
set.seed(42)
cols_good <- distinctColorPalette(48) 
...
phylo1 <- 
  tree_tbl %>% 
    ...
    geom_tippoint(aes(color = X6, shape=factor(species, levels=species_order)), size=5) +
...
apereira_save_plot(
  ...
  plot_name = "pogo_transposases_afu_afi_aoer_rootAft1", 
  limitsize = FALSE,
  scale=1,
  width     = 50, 
  height    = 20)
